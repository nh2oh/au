...
TEST(scale_12tet_tests, DefaultConstructor) {
	...
	for (int i=-200; i<200; ++i) {
		...
		frq_t curr_frq {sc.to_frq(curr_scd)};
		tf = (ntl_t{curr_ntstr} == ans_curr_ntl);
		EXPECT_TRUE(tf);
		tf = (curr_scd == sc.to_scd(curr_ntstr));
		EXPECT_TRUE(tf);
		tf = (curr_scd == sc.to_scd(curr_frq));
		EXPECT_TRUE(tf);
		tf = (curr_ntstr == sc.to_ntstr(curr_scd));
		EXPECT_TRUE(tf);
		tf = (curr_ntstr == sc.to_ntstr(curr_frq));
		EXPECT_TRUE(tf);
		tf = (curr_frq == sc.to_frq(curr_scd));
		EXPECT_TRUE(tf);
		tf = sc.isinsc(curr_frq);
		EXPECT_TRUE(tf);
	}
