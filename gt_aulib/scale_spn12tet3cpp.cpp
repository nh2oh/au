...
TEST(scale_spn12tet3_tests, DefaultConstructor) {
	spn12tet3 sc {};
	auto scda4 = sc.to_scd(ntl_t{"A"},octn_t{4});
	auto na4 = *scda4;
	EXPECT_TRUE(na4.frq == frq_t{440});
	EXPECT_TRUE(na4.ntl == ntl_t{"A"});
	EXPECT_TRUE(na4.oct == octn_t{4});
	...
	for (int i=-1000; i<1000; ++i) {
		...
		//EXPECT_TRUE(ans_curr_note == *ans_curr_scd);  // double reref => same result
...
/*
TEST(scale_spn12tet3_tests, CSharpFour330Hz) {
